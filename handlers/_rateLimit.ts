...
class RateLimit {
  ...
  public canFetch(_logger: Logger): boolean {
    // _logger.debug("Have remaining fetches in window? %s", this.remaining > 0);
    return this.remaining > 0 || this.hasExpired(_logger);
  }

  private hasExpired(_logger: Logger): boolean {
    // _logger.debug(
  ...
  public update(response: FetchResponse, _logger: Logger) {
    ...
    // _logger.debug('Response Headers: %o', responseHeaders);
    ...
    if (_logger) {
      // _logger.debug("Limit: %s", this.limit);
